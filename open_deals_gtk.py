...
'''.format(direction == -1 and u'Длинная' or u'Короткая',
    ...
    def update_deals_tab(self):
        ...
        for (did,) in self.database.connection.execute("select id from deals where parent_deal_id is null and ({0}) {1}".format(self.pick_up_filter_condition, self.deals_order_by)):
