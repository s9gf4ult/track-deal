...
class sqlite_model(common_model):
    ...
    def _calculate_statistics(self, aid):
        ...
        for query, name in [('select count(*) from deals where account_id = ? and parent_deal_id is null', 'deals_count'),
                            ...
                            ('select count(*) from positions_view where account_id = ? and direction = -1', 'long_positions_count'),
                            ('select count(*) from positions_view where account_id = ? and direction = 1', 'short_positions_count'),
                            ('select max(pl_net) from positions_view where account_id = ? and direction = -1 and pl_net >= 0', 'long_max_profit'), 
                            ('select -min(pl_net) from positions_view where account_id = ? and direction = -1 and pl_net < 0', 'long_max_loss'), 
                            ('select sum(open_volume + close_volume) from positions_view where account_id = ? and direction = -1', 'long_volume'),
                            ('select max(pl_net) from positions_view where account_id = ? and direction = 1 and pl_net >= 0', 'short_max_profit'), 
                            ('select -min(pl_net) from positions_view where account_id = ? and direction = 1 and pl_net < 0', 'short_max_loss'), 
                            ('select sum(open_volume + close_volume) from positions_view where account_id = ? and direction = 1', 'short_volume'),
                            ('select avg(pl_net) from positions_view where account_id = ? and direction = -1 and pl_net >= 0', 'long_profit_average'), 
                            ('select -avg(pl_net) from positions_view where account_id = ? and direction = -1 and pl_net < 0', 'long_loss_average'), 
                            ('select avg(pl_net) from positions_view where account_id = ? and direction = 1 and pl_net >= 0', 'short_profit_average'), 
                            ('select -avg(pl_net) from positions_view where account_id = ? and direction = 1 and pl_net < 0', 'short_loss_average'), 
                            ('select avg(pl_net) from positions_view where account_id = ? and direction = -1', 'long_plnet_average'),
                            ('select avg(pl_net) from positions_view where account_id = ? and direction = 1', 'short_plnet_average'), 
                            ...
                            ('select avg(pl_net) from positions_view where account_id = ? and pl_net >= 0', u'profit_average'),
                            ('select -avg(pl_net) from positions_view where account_id = ? and pl_net < 0', u'loss_average'),
